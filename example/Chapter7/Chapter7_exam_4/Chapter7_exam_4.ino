/***********************************************************************
 * Project      :     smartbuilding360hub Water Leak Sensor OLED + Alert + Relay
...
{
    Serial.printf("\r\n***********************************************************************\r\n");
    Serial.printf("* Project      :     smartbuilding360hub Water Leak Sensor OLED + Alert + Relay\r\n");
...
void setup()
{
    ...
    mcu.displayOLEDLines("Water: Ready", "Relay: OFF");
}
...
void loop()
{
    ...
    // แสดงสถานะเซนเซอร์น้ำรั่วทั้งบน Serial และ OLED และควบคุม Relay
    if (waterLeakState == HIGH) // ตรวจจับน้ำรั่ว
    {
        Serial.println("Water Leak Detected!"); // แสดงข้อความทาง Serial
        snprintf(line1, sizeof(line1), "Water: Leak!"); // แสดงข้อความบน OLED
        snprintf(line2, sizeof(line2), "Status: ALERT");
        mcu.beep(2, 100); // เพิ่มเสียงแจ้งเตือนเมื่อพบการรั่วของน้ำ
        mcu.relayOn();    // เปิดรีเลย์เมื่อพบการรั่วของน้ำ
        relayState = true;
    }
    else // ไม่พบการรั่วของน้ำ
    {
        Serial.println("Water: No Leak"); // แสดงข้อความทาง Serial
        snprintf(line1, sizeof(line1), "Water: No Leak"); // แสดงข้อความบน OLED
        snprintf(line2, sizeof(line2), "Status: NORMAL");
        mcu.relayOff();   // ปิดรีเลย์เมื่อไม่พบการรั่วของน้ำ
        relayState = false;
    }

/***********************************************************************
 * Project      :     smartbuilding360hub Vibration Sensor
...
{
    Serial.printf("\r\n***********************************************************************\r\n");
    Serial.printf("* Project      :     smartbuilding360hub Vibration Sensor\r\n");
...
void loop()
{
    int16_t raw = mcu.readADCChannel(0);   // อ่านค่า ADC จากช่อง AIN0
    float voltage = raw * 0.1875 / 1000.0; // แปลงค่าดิบเป็นโวลต์
    ...
    char line1[22], line2[22]; // สร้างตัวแปรสำหรับเก็บข้อความที่จะแสดงบน OLED

    // ตรวจสอบว่าค่าดิบเกิน threshold หรือไม่
    if (abs(raw) > threshold) // ใช้ abs() เพื่อเปรียบเทียบค่าบวกและลบ
    {
        Serial.println("*** Vibration detected! ***");
        snprintf(line1, sizeof(line1), "Vib! Raw:%d", raw); // แสดงข้อความเมื่อตรวจพบการสั่นสะเทือน
        snprintf(line2, sizeof(line2), "V:%.2f T:%d", voltage, threshold); // แสดงค่าโวลต์และ threshold
        mcu.displayOLEDLines(line1, line2); // แสดงบน OLED
        mcu.beep(2, 200);                   // ส่งเสียงเตือน 2 ครั้ง (deep)
    }
    else
    {
        snprintf(line1, sizeof(line1), "Raw:%d V:%.2f", raw, voltage); // แสดงค่าดิบและโวลต์
        snprintf(line2, sizeof(line2), "Threshold: %d", threshold); // แสดงค่า threshold
        mcu.displayOLEDLines(line1, line2); // แสดงบน OLED
    }
